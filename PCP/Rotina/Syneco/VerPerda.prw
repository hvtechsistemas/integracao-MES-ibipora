...
/*/{Protheus.doc} VerPerda
Função para verificar se a ordem de produção enviada para o Syneco teve perda. (via Job ou Menu)
@type  Function
@author Bruno Aguiar
@since 07/08/2024
*/
 
User Function VerPerda()
    ...
    If IsBlind()
       integracaoSyneco.U_ReenviaPerda(1)
    Else
        ValidPerg()
	    cDeFilial      	:= MV_PAR01
	    cAteFilial     	:= MV_PAR02
	    cDeOP     	    := MV_PAR03
	    cAteOP     	    := MV_PAR04
        integracaoSyneco.U_ReenviaPerda(2, cDeFilial, cAteFilial, cDeOP, cAteOP)
    EndIf
...
Static Function ValidPerg()
    Local cGrupoPerg	    := "Enviar Perda Ordem de Producao"             
    Local cTituloPerg	    := "Enviar Perda Ordem de Producao" 
    Local aRet 			    := {} 
    Local aPergs		    := {}
    Local lRet			    := .F.
